...
public class WebSocketService : IWebSocketService, IDisposable
{
    ...
    public Task HandleWebSocketConnectionAsync(WebSocket webSocket, Guid userId, string userType)
    {
    ...
    private void OnSocketClosed(IWebSocketConnection socket)
    {
        ...
        if (connection.Key != Guid.Empty) _connectionManager.RemoveConnection(connection.Key);
    }
    ...
    public async Task ProxyToFleckAsync(WebSocket webSocket, string fleckUrl)
    {
        ...
        try
        {
            ...
            var receiveTask = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        var result = await clientWebSocket.ReceiveAsync(new ArraySegment<byte>(buffer),
                            cancellationTokenSource.Token);
                        if (result.MessageType == WebSocketMessageType.Close)
                        {
                            _logger.LogInformation("Fleck WebSocket closed the connection.");
                            await webSocket.CloseAsync(WebSocketCloseStatus.NormalClosure, "Closed by Fleck",
                ...
                try
                {
                    while (webSocket.State == WebSocketState.Open)
                    {
                        ...
                        if (result.MessageType == WebSocketMessageType.Close)
                        {
                            ...
                            await clientWebSocket.CloseAsync(WebSocketCloseStatus.NormalClosure, "Closed by client",
                                CancellationToken.None);
                            break;
                        }
        ...
        catch (System.Net.WebSockets.WebSocketException ex)
        {
            ...
            await webSocket.CloseAsync(WebSocketCloseStatus.InternalServerError, "WebSocket error",
                CancellationToken.None);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error occurred while proxying WebSocket connection.");
            await webSocket.CloseAsync(WebSocketCloseStatus.InternalServerError, "Unexpected error",
                CancellationToken.None);
        }
        finally
        {
            ...
            if (clientWebSocket.State == WebSocketState.Open || clientWebSocket.State == WebSocketState.CloseReceived)
                await clientWebSocket.CloseAsync(WebSocketCloseStatus.NormalClosure, "Proxy closed",
                    CancellationToken.None);
