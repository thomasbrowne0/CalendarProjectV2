...
public interface ICalendarAppService
{
    ...
    Task<IEnumerable<CalendarEventDto>> GetEventsByCompanyIdAsync(Guid companyId, DateTime? startDate,
        DateTime? endDate);

    Task<IEnumerable<CalendarEventDto>> GetEventsByEmployeeIdAsync(Guid employeeId, DateTime? startDate,
        DateTime? endDate);
